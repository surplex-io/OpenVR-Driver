...
websocket_trackersDriver::ControllerDevice::ControllerDevice(std::string serial, ControllerDevice::Handedness handedness):
    serial_(serial),
    handedness_(handedness)
{
}

std::string websocket_trackersDriver::ControllerDevice::GetSerial()
{
    return this->serial_;
}

void websocket_trackersDriver::ControllerDevice::Update()
{
...
DeviceType websocket_trackersDriver::ControllerDevice::GetDeviceType()
{
    return DeviceType::CONTROLLER;
}

websocket_trackersDriver::ControllerDevice::Handedness websocket_trackersDriver::ControllerDevice::GetHandedness()
{
    return this->handedness_;
}

vr::TrackedDeviceIndex_t websocket_trackersDriver::ControllerDevice::GetDeviceIndex()
{
    return this->device_index_;
}

vr::EVRInitError websocket_trackersDriver::ControllerDevice::Activate(uint32_t unObjectId)
{
    ...
    GetDriver()->GetProperties()->SetStringProperty(props, vr::Prop_ModelNumber_String, "websocket_trackers_controller");

    // Set up a render model path
    GetDriver()->GetProperties()->SetStringProperty(props, vr::Prop_RenderModelName_String, "{websocket_trackers}websocket_trackers_controller");
    ...
    GetDriver()->GetProperties()->SetStringProperty(props, vr::Prop_InputProfilePath_String, "{websocket_trackers}/input/websocket_trackers_controller_bindings.json");
    ...
    std::string controller_ready_file = "{websocket_trackers}/icons/controller_ready_" + controller_handedness_str + ".png";
    std::string controller_not_ready_file = "{websocket_trackers}/icons/controller_not_ready_" + controller_handedness_str + ".png";
...
void websocket_trackersDriver::ControllerDevice::Deactivate()
{
    this->device_index_ = vr::k_unTrackedDeviceIndexInvalid;
}

void websocket_trackersDriver::ControllerDevice::EnterStandby()
{
}

void* websocket_trackersDriver::ControllerDevice::GetComponent(const char* pchComponentNameAndVersion)
{
    return nullptr;
}

void websocket_trackersDriver::ControllerDevice::DebugRequest(const char* pchRequest, char* pchResponseBuffer, uint32_t unResponseBufferSize)
{
    if (unResponseBufferSize >= 1)
        pchResponseBuffer[0] = 0;
}

vr::DriverPose_t websocket_trackersDriver::ControllerDevice::GetPose()
{
    return last_pose_;
}
